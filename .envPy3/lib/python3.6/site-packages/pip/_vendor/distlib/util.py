...
class Cache(object):
    """
    A class_mod implementing a cache for resources that need to live in the file system
    e.g. shared libraries. This class_mod was moved from resources to here because it
...
#
    ...
    class HTTPSHandler(BaseHTTPSHandler):
        ...
        def _conn_maker(self, *args, **kwargs):
            ...
            pass a connection class_mod to do_open, but it doesn't actually check for
            a class_mod, and just expects a callable. As long as we behave just as a
            constructor would have, we should be OK. If it ever changes so that
            we *must* pass a class_mod, we'll create an UnsafeHTTPSConnection class_mod
            which just sets check_domain to False in the class_mod definition, and
    ...
    # you can use the following handler class_mod, which does not allow HTTP traffic.
    #
